...
veryOldInventors=inventors.filter((age) => {
...
inventorNames=inventors.map((namef) => {
    return { first: namef.first, last: namef.last };
});
...
firstLast=people.map((name)=>{
    ...
    return{first: splitName[1]+ " "+splitName[0]}
...
let commentById = {};
commentById=comments.find((IDe)=>{
    return IDe.id===823423;
...
// Complete the exercise in the space below:
idx=comments.findIndex((inDXe)=>{
    return inDXe.id===123523
});
// Check your work:
